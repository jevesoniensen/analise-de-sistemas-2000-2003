...
public class Calculos
{
	...
	public Calculos(Vector tarefas,int numtarefas, boolean fazCalculos)
	{
	...
	public Vector retornaCiclos()
	{
		boolean existciclo  = false;
		Vector vet_caminhos = new Vector();
		int            cols = 0,
		              teste = 0;
			...
			for(int j = 0;j < numtarefas;j++)
			{
				...
				{		
					...
					for(int l = 0; l < numtarefas; l++)
					{
						...
						if(j != teste)										
						{
	   						if(!vet_caminhos.contains(new Integer(cols)))	   					
	   						{
	   							vet_caminhos.add(new Integer(cols));		   							   					
	   						}
	   					}
						else
						{ 
